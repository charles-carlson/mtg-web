 import {
    Component, ElementRef, computed, inject, output, signal, viewChild,
    type WritableSignal,
  } from '@angular/core';
...
export interface CardFilter { set: string; colors: string[]; rarity: string[]; }
...
export class CardFilterDropDown {
  ...
  open() { this.drawer().nativeElement.showModal(); }
  close() { this.drawer().nativeElement.close(); }

  private toggle(sig: WritableSignal<string[]>, value: string) {
    sig.update(cur => cur.includes(value) ? cur.filter(v => v !== value) : [...cur, value]);
  }
  toggleColor(c: string) { this.toggle(this.selectedColors, c); }
  toggleRarity(r: string) { this.toggle(this.selectedRarities, r); }
  selectSet(s: string) { this.selectedSet.set(s); }

  clearAll() {
    this.selectedColors.set([]); this.selectedRarities.set([]); this.selectedSet.set('');
  }

  onApply() {
    this.apply.emit({
      set: this.selectedSet(), colors: this.selectedColors(), rarity: this.selectedRarities(),
    });
    this.close()
  }
